...
//! single-pass over a raw fd, with all five compression backends `build.rs` vendors compiled in
//! (lz4, zstd, zlib, bzip2 and xz -- M3.1 parts 2a-2e, added one at a time, each proven by a
//! fixture pair under `core/fixtures/` generated by `tools/fixtures/make_compression_fixtures.py`)
//! and no random access (M3.2's own stated scope). M3.1 part 3a adds [policy], the extraction
//! policy ported from the app's `ArchiveExtractionPolicy.kt` and the source of truth for it from
//! here on; `extract` and `inspect` (part 3b) follow.
